...
export function useContentEditable() {
    ...
    function insertTextAtCursor(
        ...
        while ((match = regex.exec(value)) !== null) {
            ...
            const afterMatch = value.substring(matchStart);

            // Check if this {{ has a matching }}
            const closingMatch = afterMatch.match(/\}\}/);
            if (!closingMatch) {
        ...
        // Find where the incomplete pattern ends
        // Look for the next space, newline, or end of string after {{
        const textAfterIncompletePattern = value.substring(lastIncompleteMatch + 2);
        const patternEndMatch = textAfterIncompletePattern.match(/^[^\s\n]*/);
