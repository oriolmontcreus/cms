...
export async function getRedisClient(): Promise<RedisClientType | null> {
  ...
  if (useInMemoryFallback || redisConnectionAttempted) return null;
 ...
 */
export async function storeWithExpiry<T>(
  key: string, 
  value: T, 
  expirySeconds: number,
  fallbackMap: Map<string, { value: T; expiresAt: number }>
): Promise<void> {
  const redis = await getRedisClient();
 ...
 * Falls back to the provided Map if Redis is unavailable
 */
export async function retrieveWithExpiry<T>(
  key: string,
  fallbackMap: Map<string, { value: T; expiresAt: number }>
): Promise<T | null> {
  const redis = await getRedisClient();
...
export async function deleteKey(
  key: string,
  fallbackMap: Map<string, unknown>
): Promise<void> {
  const redis = await getRedisClient();
