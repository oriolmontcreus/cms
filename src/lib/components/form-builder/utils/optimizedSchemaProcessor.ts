...
function extractAllFields(schema: Layout | SchemaItem[]): FormField[] {
    ...
    function traverse(items: any) {
        ...
        if (field) {
            ...
            // Handle nested schema in repeatable fields
...
export function initializeTranslationDataOptimized(
    ...
    for (const componentInstance of components) {
        const { translatableFields, repeatableFields } = processComponentSchema(componentInstance.component);
        const existingComponent = existingLookup.get(componentInstance.id);
        ...
        for (const locale of locales) {
            ...
            // Handle repeatable fields for non-CMS locales
            if (locale.code !== CMS_LOCALE) {
                for (const repeatableField of repeatableFields) {
                    ...
                    const nestedTranslatableFields = extractAllFields(repeatableField.schema || [])
    ...
    if ('type' in item) {
        if (item.type === SCHEMA_TYPES.GRID && 'schema' in item) {
            return (item.schema as FormField[]).some(f => 
                f.translatable === true || 
                (f.type === 'repeatable' && f.schema && hasTranslatableFieldsInSchema(f.schema))
            );
        }
